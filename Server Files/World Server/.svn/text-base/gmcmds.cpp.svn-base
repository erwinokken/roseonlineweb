...
//		ADDSTRING( pak, "DELETESPAWN" );
    ...
    else if (strcmp(command, "go")==0) // AtCommandGo
    {
        ...
        {
             if (thisclient->Stats->Level<110)
             {
             SendPM(thisclient, "You need to be a least Level 110 to visit Eldeon!");
             return true;
             }
         {
            map = 61;
        ...
        else if (loc == 12) // The Wasteland of Orlo
        {
             if (thisclient->Stats->Level<180)
             {
                 SendPM(thisclient, "You need to be a least Level 195 to visit The Wasteland of Orlo!");
                 return true;
             }
             else
             {
       ...
       else if (loc == 13) // Desert City of Muris
        {
             if (thisclient->Stats->Level<205)
             {
                 SendPM(thisclient, "You need to be a least Level 205 to visit Desert City of Muris!");
                 return true;
             }
             else
             {
        ...
        else
        {
           ...
           SendPM(thisclient, "10 = Oblivion Temple");
            ...
            SendPM(thisclient, "12 = The Wasteland of Orlo");
            SendPM(thisclient, "13 = Desert City of Muris");
...
//         }
   ...
   if(Config.Command_Set > thisclient->Session->accesslevel)
        ...
        if(EquipList[2].Index[id]!=NULL)
        {
            ...
            thisclient->items[2].socketed = sloted;
        ...
        if(EquipList[3].Index[id]!=NULL)
        {
            ...
            thisclient->items[3].socketed = sloted;
        ...
        if(EquipList[4].Index[id]!=NULL)
        {
            ...
            thisclient->items[5].socketed = sloted;
        ...
        if(EquipList[5].Index[id]!=NULL)
        {
            ...
            thisclient->items[6].socketed = sloted;
...
bool CWorldServer::pakGMReborn(CPlayer* thisclient)
{
    ...
    if(thisclient->Party->party!=NULL)
    {
     ...
     if(thisclient->Stats->Level < 240) //Level 250 can be changed to any level you want
     {
        GServer->SendPM(thisclient, "You have to be lvl 240 to reborn !");
     }
     else
     {

        GServer->DB->QExecute("UPDATE characters SET reborns=reborns+'1' WHERE id='%i'",thisclient->CharInfo->charid);
        GServer->DB->QExecute("UPDATE accounts SET reborns=reborns+'1' WHERE id='%i'",thisclient->Session->userid);
