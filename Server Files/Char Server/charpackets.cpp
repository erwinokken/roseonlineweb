...
bool CCharServer::pakCreateChar( CCharClient* thisclient, CPacket* P )
{
	...
	if(!DB->QExecute("INSERT INTO items VALUES(%i,1,8,0,40,100,7,1,0,0,0,0,0)", mid))
		return false;
    if(!DB->QExecute("INSERT INTO items VALUES(%i,449,6,144,40,100,18,1,0,1,1,0,0)", mid))
        return false;
    if(!DB->QExecute("INSERT INTO items VALUES(%i,1,1,144,40,100,13,1,0,1,1,0,0)", mid))
        return false;
    if(!DB->QExecute("INSERT INTO items VALUES(%i,315,10,0,100,100,42,10,0,0,0,0,0)", mid))
        return false;
    if(!DB->QExecute("INSERT INTO items VALUES(%i,316,10,0,100,100,43,10,0,0,0,0,0)", mid))
        return false;
    if(!DB->QExecute("INSERT INTO items VALUES(%i,317,10,0,100,100,44,10,0,0,0,0,0)", mid))
        return false;
    if(!DB->QExecute("INSERT INTO items VALUES(%i,318,10,0,100,100,45,10,0,0,0,0,0)", mid))
        return false;
